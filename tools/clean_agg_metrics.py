...
def clean_agg_metrics_data(df: pd.DataFrame, run_dir_name: str, logger: logging.Logger) -> Tuple[pd.DataFrame, dict]:
    """
    ...
    """
    stats = {"original_rows": len(df), "rows_removed_stac_mismatch": 0, "rows_removed_duplicates": 0, "final_rows": 0}
    ...
    if missing_cols:
        ...
        if duplicated_mask.any():
            ...
            if "true_positives_count" in df.columns and "false_positives_count" in df.columns:
                df_dups["combined_score"] = df_dups["true_positives_count"].fillna(0) + df_dups[
...
def process_agg_metrics_file(
    file_path: str, aws_profile: str, logger: logging.Logger, dry_run: bool = False
) -> Optional[dict]:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Clean agg_metrics.csv files in S3 batch directories")
    ...
    parser.add_argument("--batch", required=True, help="Batch directory name (e.g., fim100_huc12_5m_non_calibrated)")

    parser.add_argument(
        "--log-file", default="agg_metrics_cleaning.log", help="Path to log file (default: agg_metrics_cleaning.log)"
    )

    parser.add_argument("--profile", default="fimbucket", help="AWS profile for S3 access (default: fimc-data)")
    ...
    try:
        ...
        # Process each file
        total_stats = {"files_processed": 0, "files_modified": 0, "total_rows_removed": 0}

        for file_path in agg_files:
            stats = process_agg_metrics_file(file_path, args.profile, logger, args.dry_run)
