...
from default_config import (
    NOMAD_ADDRESS, NOMAD_TOKEN, NOMAD_NAMESPACE, 
    AGREEMENT_MAKER_JOB_NAME, FIM_MOSAICKER_JOB_NAME, HAND_INUNDATOR_JOB_NAME
)
...
Default pipeline jobs: {', '.join(PIPELINE_JOBS)}
    ...
    parser.add_argument(
        '--nomad-addr',
        default=os.getenv('NOMAD_ADDRESS', NOMAD_ADDRESS),
        help=f'Nomad server address (default: {NOMAD_ADDRESS})'
    )
    parser.add_argument(
        '--nomad-token',
        default=os.getenv('NOMAD_TOKEN', NOMAD_TOKEN),
        help='Nomad authentication token'
    )
    parser.add_argument(
        '--namespace',
        default=os.getenv('NOMAD_NAMESPACE', NOMAD_NAMESPACE),
    ...
    purger = NomadPurger(
        ...
        token=args.nomad_token if args.nomad_token else None,
