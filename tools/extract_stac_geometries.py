...
def extract_geometry_by_stac_id(
    ...
    # Create GeoDataFrame
    gdf = gpd.GeoDataFrame([{"item_id": item_id, "collection": item.collection_id}], geometry=[geom], crs="EPSG:4326")
...
def main():
    ...
    parser.add_argument(
        "--stac-api-url", default="http://benchmark-stac.test.nextgenwaterprediction.com:8000", help="STAC API URL"
    )
    ...
    for idx, item_id in enumerate(item_ids):
        try:
            ...
            gdf = extract_geometry_by_stac_id(
                item_id, stac_api_url=args.stac_api_url, collection=args.collection, use_convex_hull=False
            )
            ...
            if should_use_convex_hull(collection_id):
                ...
                gdf = extract_geometry_by_stac_id(
                    item_id, stac_api_url=args.stac_api_url, collection=args.collection, use_convex_hull=True
                )
