...
class NomadApiClient:
    ...
    def __init__(self, config: AppConfig, session: aiohttp.ClientSession): # Accept session
        ...
        self.events = self.EventsWrapper(self._session, self.base_url, self.namespace)
        self.config = config
    ...
    async def dispatch_job(self, job_name: str, job_instance_id: str, meta: Dict) -> Dict:
        """Dispatches a pre-registered parameterized job with retries."""
        url = f"{self.base_url}/v1/job/{job_name}/dispatch"
        params = {'namespace': self.namespace} if self.namespace and self.namespace != '*' else {}
        ...
        try:
            ...
            async with self._session.post(url, json=payload, params=params, headers=self._headers, timeout=30) as response:
                # Check status before parsing JSON
                if response.status >= 400:
                    # Log specific error but let raise_for_status handle retry/reraise logic
                    logging.warning(f"Nomad API response status {response.status} for dispatch '{job_name}'.")
                ...
                if not dispatched_id:
                    raise ValueError(f"Dispatch response for {job_name} missing DispatchedJobID")
                logging.info(f"Job '{job_name}' dispatched. EvalID: {response_data.get('EvalID')}, DispatchedJobID: {dispatched_id}")
                return response_data
        except RetryError as e:
            # Logged when retries are exhausted
            logging.error(f"Dispatch failed for job '{job_name}' after multiple retries: {e.last_attempt.exception()}")
            raise ConnectionError(f"Failed to dispatch job {job_name} after retries") from e.last_attempt.exception()
        except aiohttp.ClientResponseError as e:
            status = e.status; message = e.message; body = "";
            if response and response.content: with suppress(Exception): body = await response.text()
            # Raise more specific standard errors for non-retryable client errors
            if status == 400: raise ValueError(f"Nomad bad request dispatching {job_name}: {message}. Body: {body[:200]}") from e
            if status == 401 or status == 403: raise PermissionError(f"Nomad permission error dispatching {job_name}: {message}") from e
            if status == 404: raise LookupError(f"Nomad job '{job_name}' not found: {message}") from e
            # If it wasn't retried (e.g., 4xx), raise a ConnectionError or let original propagate
            logging.error(f"Nomad API error dispatching job '{job_name}' ({status}): {message}. Body: {body[:500]}...")
            raise ConnectionError(f"Nomad API error dispatching job {job_name} (status {status})") from e
        except aiohttp.ClientConnectionError as e:
             logging.error(f"Connection error dispatching job '{job_name}' to Nomad: {e}")
             raise ConnectionError(f"Nomad connection error dispatching job {job_name}") from e
        except asyncio.TimeoutError as e:
             logging.error(f"Timeout dispatching job '{job_name}' to Nomad.")
             raise TimeoutError(f"Timeout dispatching job {job_name}") from e # Standard TimeoutError
    ...
    class EventsWrapper:
        ...
        async def stream(self, topics: Dict = None) -> AsyncGenerator[Dict, None]:
            ...
            response: Optional[aiohttp.ClientResponse] = None
            try:
                response = await self._session.get(url, params=params, timeout=None)
